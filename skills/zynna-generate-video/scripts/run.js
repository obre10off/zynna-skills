...
async function confirmGeneration(args) {
  console.log('About to generate video via Zynna Open Skills proxy.');
...
async function main() {
  ...
  if (!args.runId || (!args.prompt && !args.taskId)) {
    console.error('Usage: run.js --run_id <run_id> (--prompt <prompt> [--ratio 9:16] [--model kling-2.6] [--yes] | --task_id <task_id> [--wait] [--model kling-2.6])');
    process.exit(2);
  }
