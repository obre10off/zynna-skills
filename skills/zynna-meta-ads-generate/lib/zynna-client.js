...
class ZynnaOpenSkillsClient {
  ...
  async submitImageTask(taskBody, timeoutSec = 300) {
    const payload = await this.requestJson(
      'POST',
      `${this.cfg.baseUrl}/api/open/skills/image/tasks`,
      { body: taskBody, timeoutSec }
    );
    this.assertApiSuccess(payload, 'image task submit');
    return payload.data || {};
  }

  async estimateImageTask(taskBody, timeoutSec = 120) {
    const payload = await this.requestJson(
      'POST',
      `${this.cfg.baseUrl}/api/open/skills/image/tasks/estimate`,
      { body: taskBody, timeoutSec }
    );
    this.assertApiSuccess(payload, 'image estimate');
    return payload.data || {};
  }

  async getImageTaskStatus(taskId, timeoutSec = 60) {
    const payload = await this.requestJson(
      'GET',
      `${this.cfg.baseUrl}/api/open/skills/image/tasks/status?task_id=${encodeURIComponent(taskId)}`,
      { timeoutSec }
    );
    this.assertApiSuccess(payload, 'image status');
    return payload.data || {};
  }
}
